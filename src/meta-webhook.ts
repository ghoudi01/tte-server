...
import { getMerchantMetaConnectionByPageOrInstagramId, getMerchantMetaBotEnabled } from "./store";
...
export function registerMetaWebhookRoutes(app: Express): void {
  ...
  app.post("/api/webhooks/meta", async (req: Request, res: Response) => {
    ...
    for (const entry of body.entry ?? []) {
      ...
      const botEnabled = await getMerchantMetaBotEnabled(conn.merchant_id);
      ...
      for (const ev of entry.messaging ?? []) {
        ...
        void handleInboundMessaging({
          ...
          console.error(`[meta-webhook] Error processing inbound ${channel} message:`, err);
        });
      }
    }
  });
