    ...
    await withMerchant(req, res, "POST /api/plugin/webhooks/shopify", async merchantId => {
      ...
      if (body.id && body.customer) {
        try {
          ...
          const shipping = body.shipping_address as Record<string, unknown> | undefined;
          await createOrder({
            merchantId,
            customerName: `${customer.first_name ?? ""} ${customer.last_name ?? ""}`.trim(),
            phoneNumber: (customer.phone as string) ?? (shipping?.phone as string) ?? "",
          ...
          console.error("[plugin-api] Shopify order creation failed:", e);
        }
      }
      ...
      if (body.id && body.billing) {
        try {
          ...
          await createOrder({
            ...
            phoneNumber: (billing.phone as string) ?? "",
            city: billing.city as string | undefined,
            orderAmount: Number(body.total ?? 0),
            status: "pending",
            verificationStatus: "pending",
          });
        } catch (e) {
          console.error("[plugin-api] WooCommerce order creation failed:", e);
        }
      }
      res.json({ ok: true, synced: true });
    });
